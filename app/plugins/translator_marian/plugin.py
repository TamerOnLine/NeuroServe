...
_LANG_ALIASES = {
    ...
    "en":"english","english":"english","انجليزي":"english","الانجليزية":"english"
}
...
class Plugin(AIPlugin):
    ...
    def _once(self, text:str, name:str, max_new:int, num_beams:int, do_sample:bool,
              temperature:Optional[float], top_p:Optional[float]) -> str:
        ...
        gen = dict(max_new_tokens=max_new, num_beams=(1 if do_sample else max(1,num_beams)),
                   do_sample=do_sample, early_stopping=True)
        if do_sample:
            if temperature is not None: gen["temperature"] = float(temperature)
            if top_p is not None: gen["top_p"] = float(top_p)
        with torch.no_grad():
    ...
    def infer(self, payload:dict) -> dict:
        ...
        try:
            ...
            if getattr(self.dev,"type","")=="cuda": torch.cuda.synchronize()
            return {
                "task":"translate","provider":"translator_marian","device":str(self.dev),
                "model":name,"backend":"marian","params":{
                    "source_lang":src,"target_lang":tgt,"max_new_tokens":max_new,
                    "do_sample":do_sample,"num_beams":num_beams,
                    "temperature":temperature,"top_p":top_p
                },"input_chars":len(text),"output":out,"elapsed_sec":round(time.time()-t0,3)
            }
